...
internal static class FastMemberAccessor<TClass, TValue> where TClass : class
{
    ...
    public static void Set([CanBeNull] TClass instance, string memberName, TValue value)
    {
        if (!_setters.TryGetValue(memberName, out var setter))
        {
            ...
            var memberExpression = Expression.MakeMemberAccess(isStatic ? null : instanceParameterExpression, member);
            var assignExpression = AssignmentExpression.Create(memberExpression, valueParameterExpression);
...
internal static class AssignmentExpression
{
    public static Expression Create(Expression left, Expression right)
    {
        return Expression.Call(
            null,
            typeof(AssignmentExpression).GetMethod(nameof(AssignTo), BindingFlags.NonPublic | BindingFlags.Static)!
